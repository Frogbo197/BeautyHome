...
class DailySummaryService
{
    public function calculate(int $userId, string $date): array
    {
        return [
            ...
            'ActivityMinutes' => $this->activityMinutes($userId, $date),
        ];
    }
    ...
    public function refresh(int $userId, string $date): array
    {
        ...
        if (!Schema::hasTable('daily_health_summary')) {
            return $totals;
        }

        $payload = [
            'TotalCaloriesIn' => $totals['CaloriesIn'],
            'TotalCaloriesOut' => $totals['CaloriesOut'],
            'NetCalories' => $totals['CaloriesIn'] - $totals['CaloriesOut'],
            'TotalWaterML' => $totals['WaterML'],
            'TotalSteps' => $totals['Steps'],
            'ActivityMinutes' => $totals['ActivityMinutes'],
            'GoalStatus' => $totals['CaloriesIn'] >= $totals['CaloriesOut'] ? 'balanced' : 'deficit',
            'CreatedAt' => now(),
        ];
        ...
        foreach ([
            ...
            if (Schema::hasColumn('daily_health_summary', $column)) {
                $payload[$column] = $value;
            }
        }

        DB::table('daily_health_summary')->updateOrInsert(
